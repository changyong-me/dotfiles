...
1. Inspect the issue and stop if it is not agent-ready.
2. Create a branch and open an empty PR if there is no existing relevant PR.
3. Spawn the implementer subagent, then just wait for it to finish.
4. Post the reported decisions as a PR comment without rewriting them.
5. Spawn the reviewer subagents in parallel, then just wait for both to finish.
6. Post the reported reviews as a PR comment without merging or rewriting them.
...
The implementer subagent should inspect any relevant context and create a goal; then implement, validate, commit, push, and report the result including the decisions the issue left open.
